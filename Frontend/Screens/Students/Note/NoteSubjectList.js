  ...
  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        ...
        const data = res.data;
        const theory = Array.isArray(data.subjects)
          ? data.subjects.map((name) => ({ name, type: 'Theory' }))
          : [];
        const practical = Array.isArray(data.labs)
          ? data.labs.map((name) => ({ name, type: 'Lab' }))
          : [];
